...
CLASSIFIER_DIR = TOOLS_DIR.parent
TARGET_DIR = CLASSIFIER_DIR / "LMCPP"
STAGING_DIR = CLASSIFIER_DIR / "staging_lmcpp"
VERSION_MANIFEST = TARGET_DIR / ".version"
RELEASES_LIST_URL = (
    f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/releases?per_page=10"
)
LATEST_RELEASE_URL = (
...
CRITICAL_DLLS = {
    "cuda": (
        ["cudart64", "cublas64", "ggml-cuda"]
        if os.name == "nt"
        else ["libcudart", "libcublas", "libggml-cuda"]
    ),
    "rocm": (
        ["rocblas", "hip", "ggml-rocm", "ggml-hip"]
        if os.name == "nt"
        else ["librocblas", "libamdhip", "libggml-rocm", "libggml-hip"]
    ),
    ...
    if path.is_dir():
        ...
        for attempt in range(5):
            try:
                ...
                try:
                    os.chmod(
                        str(path), stat.S_IWRITE if os.name == "nt" else 0o755
                    )
                    path.unlink()
                    return
                except Exception:
    ...
    try:
        ...
        return (
            subprocess.check_output(
                cmd, stderr=subprocess.DEVNULL, timeout=5, **kwargs
            )
    ...
    if os.name == "nt":
        ...
        if nv_out:
            try:
                ...
                if major_ver >= 520:
                    print_status(
                        "NVIDIA CUDA Accelerated Runtime (v12)", "SUCCESS"
                    )
                    return {"type": "cuda", "ver": "12"}
                if major_ver >= 450:
                    print_status(
                        "NVIDIA CUDA Accelerated Runtime (v11)", "SUCCESS"
                    )
                    return {"type": "cuda", "ver": "11"}
            except ValueError:
                pass

        cim_out = safe_subprocess(
            [
                "powershell",
                "-NoProfile",
                "-Command",
        ...
        if "intel" in cim_out and (
            "arc" in cim_out or "iris" in cim_out or "ultra" in cim_out
        ):
...
def fetch_json(url: str) -> dict | list:
    headers = {"User-Agent": "HitBoy-LMCPP-Hydrator"}
    ...
    for attempt in range(3):
        try:
            ...
            if e.code == 403:
                print_status(
                    "GitHub API Rate Limit. Export GITHUB_TOKEN environment variable.",
                    "ERROR",
                )
                sys.exit(1)
            time.sleep(2)
...
def get_latest_binary_release() -> dict:
    ...
    fallback_data = fetch_json(LATEST_RELEASE_URL)
    if isinstance(fallback_data, dict) and fallback_data.get("assets"):
...
def download_file(
    url: str, path: Path, expected_hash: str | None = None
) -> bool:
    tmp_path = path.with_suffix(path.suffix + ".tmp")
    for attempt in range(3):
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "HitBoy-LMCPP-Hydrator"}
            )
...
def extract_source_topology(tag_name: str) -> bool:
    ...
    target_converter = root_folder / "convert_hf_to_gguf.py"
    target_gguf_py = root_folder / "gguf-py"

    if target_converter.exists():
        shutil.move(
            str(target_converter), str(TARGET_DIR / "convert_hf_to_gguf.py")
        )
    if target_gguf_py.exists():
        shutil.move(str(target_gguf_py), str(TARGET_DIR / "gguf-py"))

    for folder_name in ["conversion", "convert"]:
        target_path = root_folder / folder_name
        if target_path.exists():
            shutil.move(str(target_path), str(TARGET_DIR / folder_name))
...
def converge_structure() -> None:
    ...
    for item in TARGET_DIR.iterdir():
        if item.is_file():
            if os.name != "nt" and item.name in [
                SYSTEM_EXPECTED_SERVER,
                SYSTEM_EXPECTED_QUANT,
                SYSTEM_EXPECTED_IMATRIX,
            ]:
                os.chmod(str(item), item.stat().st_mode | stat.S_IEXEC)
            elif (
                os.name == "nt"
                and item.suffix.lower() == ".exe"
                and item.name
                not in [
                    SYSTEM_EXPECTED_SERVER,
                    SYSTEM_EXPECTED_QUANT,
...
def verify_ensemble(backend_type: str) -> tuple[bool, str]:
    ...
    if required_stubs and os.name == "nt":
        ...
        has_dependency = any(
            any(stub in dll for stub in required_stubs) for dll in found_dlls
        )
...
def select_assets(release_assets: list[dict], profile: dict) -> tuple[list[dict], str]:
    ...
    if os.name == "nt":
        if target_backend == "rocm":
            ...
            print_status(
                "No Windows ROCm archive found in release assets. Falling back to Vulkan.",
                "WARN",
            )
            for a in release_assets:
                n = a.get("name", "").lower()
                if "bin-win" in n and "vulkan" in n and "x64" in n:
                    assets_to_fetch.append(a)
            ...
            for a in release_assets:
                n = a.get("name", "").lower()
                if "cudart" in n and "bin-win" in n:
                    assets_to_fetch.append(a)
                    break
            if assets_to_fetch:
                return assets_to_fetch, "cuda"

        elif target_backend == "vulkan":
            for a in release_assets:
                n = a.get("name", "").lower()
                if "bin-win" in n and "vulkan" in n and "x64" in n:
                    assets_to_fetch.append(a)
        ...
        for a in release_assets:
            ...
            if (
                "bin-win" in n
                and ("avx2" in n or "x64" in n)
                and not any(
                    x in n for x in ["cuda", "rocm", "hip", "vulkan", "arm64"]
                )
            ):
                assets_to_fetch.append(a)
        ...
        for a in release_assets:
            n = a.get("name", "").lower()
            if "bin-macos" in n and ("arm64" in n or "universal" in n):
                assets_to_fetch.append(a)
        ...
        if target_backend == "rocm":
            for a in release_assets:
                n = a.get("name", "").lower()
                if ("ubuntu" in n or "linux" in n) and "rocm" in n:
                    assets_to_fetch.append(a)
                    return assets_to_fetch, "rocm"
        elif target_backend == "cuda":
            for a in release_assets:
                n = a.get("name", "").lower()
                if ("ubuntu" in n or "linux" in n) and "cuda" in n:
                    assets_to_fetch.append(a)
                    return assets_to_fetch, "cuda"
        for a in release_assets:
            n = a.get("name", "").lower()
            if ("ubuntu" in n or "linux" in n) and "vulkan" in n:
                assets_to_fetch.append(a)
                return assets_to_fetch, "vulkan"
        for a in release_assets:
            n = a.get("name", "").lower()
            if ("ubuntu" in n or "linux" in n) and "x64" in n:
                assets_to_fetch.append(a)
                return assets_to_fetch, "cpu"
...
def main() -> None:
    ...
    if (
        VERSION_MANIFEST.exists()
        and (TARGET_DIR / SYSTEM_EXPECTED_SERVER).exists()
    ):
        ...
        if current_version == latest_tag:
            ...
            if is_valid:
                print_status(
                    f"LMCPP engine synchronized to latest upstream ({latest_tag}).",
                    "SUCCESS",
                )
                sys.exit(0)
            else:
                print_status(
                    f"Local topology compromised ({msg}). Re-hydrating...",
                    "WARN",
                )

    print_status(f"Hydrating upstream build {latest_tag}...", "INFO")
    ...
    try:
        ...
        for asset in assets_to_fetch:
            pattern = (
                re.escape(asset["name"]) + r".*?sha256:\s*([a-fA-F0-9]{64})"
            )
            match = re.search(
                pattern, release.get("body", ""), re.DOTALL | re.IGNORECASE
            )
            expected_hash = match.group(1).lower() if match else None

            if not extract_binary_topology(
                asset["browser_download_url"], expected_hash
            ):
                raise RuntimeError(
                    f"Binary asset extraction failed: {asset['name']}"
                )
        ...
        if is_valid:
            with open(VERSION_MANIFEST, "w") as f:
                f.write(latest_tag)
            print_status(
                f"Engine Hydration Complete ({latest_tag})", "SUCCESS"
            )
        else:
